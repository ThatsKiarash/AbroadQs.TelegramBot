...
public static class CommonModuleExtensions
{
    public static IServiceCollection AddCommonModule(this IServiceCollection services)
    {
        ...
        services.AddScoped<ServerOpsHandler>();                      // Concrete registration for DI into DynamicStageHandler
        services.AddScoped<IUpdateHandler, ServerOpsHandler>(sp => sp.GetRequiredService<ServerOpsHandler>()); // SSH server operations + OpenClaw
